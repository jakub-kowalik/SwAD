...
def animate(i):
    stretcz = np.sin(np.deg2rad(i))
    stretcz2 = np.cos(np.deg2rad(i))
    ax.cla()
    ax.set_xlim((-2, 2))
    ax.set_ylim((-2, 2))
    rotated = my_fig @ stretch_by(stretcz, stretcz2, 'both')
    ax.fill(rotated[:, 0], rotated[:, 1])
    plt.draw()


ani = matplotlib.animation.FuncAnimation(fig, animate, frames=180, interval=1, repeat=True)
